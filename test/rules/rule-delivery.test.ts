...
import {
  ...
  type DeliveryPart,
} from '../../src/rules/rule-delivery-codec.js';
import type { RawHistoryAdapter, RawHistoryResult } from '../../src/rules/rule-delivery-history.js';
import { createRuleDelivery, type RuleDelivery } from '../../src/rules/rule-delivery.js';
...
describe('RuleDelivery durable turns', () => {
  ...
  it('seeds once, appends new matched rules in order, and accepts duplicate-only turns', async () => {
    ...
    const duplicateResult = await delivery.deliverDurableTurn({
      ...
      matchedRules: [{ relativePath: 'rules/first.md', content: 'First guidance.' }],
  ...
  it('rebuilds the ledger after a delivered message is removed', async () => {
    ...
    expect(replacementOutput.parts[0]?.text).toContain('Use Conventional Commits.');
  });
  ...
  it('appends deterministic identities and exact durable formats through the interface', async () => {
    ...
    await delivery.deliverDurableTurn({
      ...
      matchedRules: [{ relativePath: 'rules/core.md', content: 'Durable guidance.' }],
      output,
    });
    ...
    expect(output.parts).toEqual([
      ...
      buildDurableDeliveryPart([], [{ relativePath: 'rules/hook.md', content: 'Hook guidance.' }], {
        sessionID: 'ses_identity',
        messageID: 'msg_identity_2',
      }),
    ]);
  ...
  it('deduplicates a resumed rule by path after its content changes', async () => {
    ...
    await delivery.deliverDurableTurn({
      sessionID: 'ses_edited',
      messageID: 'msg_new',
      matchedRules: [{ relativePath: 'rules/edited.md', content: 'New content.' }],
      output,
    });
  ...
  it('scopes durable part identities to their owning message', async () => {
    ...
    for (const [index, owner] of owners.entries()) {
      ...
      await delivery.deliverDurableTurn({
        ...owner,
        matchedRules: [{ relativePath: 'rules/core.md', content: 'Shared rule guidance.' }],
  ...
  it('recognizes legacy ID-less durable markers and ignores malformed history', async () => {
    const history = new MockRawHistoryAdapter({
      ...
      messages: [
        ...
        {
          parts: [{ id: 'prt_hook_transient_zzz', type: 'text', text: 'Transient.' }],
        },
      ],
    });
    const delivery: RuleDelivery = createRuleDelivery({ rawHistory: history });
  ...
  it('serializes concurrent durable turns so first-use history is read once', async () => {
    ...
    const matchedRules = [{ relativePath: 'rules/core.md', content: 'Durable guidance.' }];
...
describe('RuleDelivery matched Hook queueing', () => {
  it('promotes durable owners, deduplicates by identity, and preserves delivery order', async () => {
    ...
    await delivery.deliverDurableTurn({
      ...
      messageID: 'msg_owner',
      matchedRules: [{ relativePath: 'rules/owner.md', content: 'Durable owner Hook.' }],
      output: { parts: [] },
    });
...
describe('RuleDelivery transient dispatches', () => {
  ...
  it('deduplicates transient rules when the agent loop reloads one user turn', () => {
    ...
    const matchedRules = [{ relativePath: 'rules/transient.md', content: 'Transient guidance.' }];
    ...
    delivery.deliverTransientDispatch({
      sessionID: 'ses_repeated',
      matchedRules: [...matchedRules, { relativePath: 'rules/new.md', content: 'New guidance.' }],
    ...
    expect(expandedDispatch[1]?.parts[0]?.text).not.toContain('Transient guidance.');
  ...
  it.each(['markCompacted', 'markHistoryChanged'] as const)(
    ...
    (invalidator) => {
      ...
      const input = () => ({
        ...
        matchedRules: [{ relativePath: 'rules/transient.md', content: 'Guidance.' }],
  ...
  it('appends matched rules, durable Hook copies, and transient Hook content in order', async () => {
    ...
    await delivery.deliverDurableTurn({
      ...
      matchedRules: [{ relativePath: 'rules/durable.md', content: 'Durable guidance.' }],
      output: { parts: [] },
    });
    delivery.queueMatchedHooks({
      sessionID: 'ses_transient',
    ...
    const messages = [
      ...
      },
      {
        ...
        parts: [{ id: 'prt_rule_ephemeral_prior', type: 'text', text: 'Prior.' }],
      },
    ];
  ...
  it('aborts a dispatch on a malformed message without appending, as before', () => {
    ...
    expect(() =>
      delivery.deliverTransientDispatch({
        ...
        matchedRules: [{ relativePath: 'rules/transient.md', content: 'Transient.' }],
        messages,
      })
    ).not.toThrow();
    expect(messages).toHaveLength(2);
