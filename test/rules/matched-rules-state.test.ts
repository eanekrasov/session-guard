...
describe('matched-rules-state', () => {
  ...
  beforeEach(async () => {
    ...
    const testDir = await fs.mkdtemp(path.join(os.tmpdir(), 'matched-rules-test-'));
  ...
  describe('write and readMatchedRulesState', () => {
    ...
    it('reads from an explicit state directory', async () => {
      const explicitStateDir = path.join(path.dirname(testStateDir), 'explicit');
    ...
    it('throws on write with invalid sessionID', () => {
      for (const sessionID of ['../escape', 'foo/bar', '/absolute', 'ses.123', 'ses 123', '']) {
        expect(() => store.write(sessionID, ['/rule.md'])).toThrow('Invalid sessionID');
      }
    });

    it('throws for read with invalid sessionID', async () => {
      for (const sessionID of ['../escape', '/absolute', 'ses.123', '']) {
        await expect(readMatchedRulesState(sessionID, { stateDir: testStateDir })).rejects.toThrow(
          'Invalid sessionID'
        );
      }
    });
    ...
    it('handles writes to different sessions independently', async () => {
      await Promise.all([store.write('ses_a', ['ruleA']), store.write('ses_b', ['ruleB'])]);
  ...
  describe('merge', () => {
    it('atomically merges admitted paths with existing state', async () => {
      ...
      expect(state?.matchedRulePaths).toEqual(['/rules/always.md', '/rules/file.md']);
    });
    ...
    it('rejects an invalid session ID before queueing', async () => {
      await expect(() => store.merge('../escape', ['x'])).toThrow(/Invalid sessionID/);
    });
  });
});

function getWriteQueues(store: MatchedRulesStateStore): Map<string, Promise<void>> {
  // Pin queue ownership without adding a production test hook.
  return (store as unknown as { writeQueues: Map<string, Promise<void>> }).writeQueues;
}
