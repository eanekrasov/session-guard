...
import { MatchedRulesStateStore } from '../../src/rules/matched-rules-state.js';
...
Use React best practices for components.`
    ...
    await chatMessage({ sessionID: testSessionID, messageID: 'msg_glob_1' }, output);
...
Rust unsafe guidance.`
    ...
    await chatMessage({ sessionID: 'ses_rs_ok', messageID: 'msg_rs_ok' }, output);
    const syntheticText = output.parts
      .filter((p) => p.synthetic)
      .map((p) => p.text)
      .join('\n');
...
Rust unsafe guidance.`
    ...
    await chatMessage({ sessionID: 'ses_rs_split', messageID: 'msg_rs_split' }, output);
...
Cleanup todos.`
    ...
    await chatMessage({ sessionID: 'ses_todo', messageID: 'msg_todo_1' }, output);
    const syntheticText = output.parts
      .filter((p) => p.synthetic)
      .map((p) => p.text)
      .join('\n');
...
Never delivered.`
    ...
    try {
      ...
      await chatMessage({ sessionID: 'ses_broken', messageID: 'msg_broken' }, output);
      expect(output.parts.filter((p) => p.synthetic)).toHaveLength(0);
    } finally {
...
Prose-only guidance.`
        ...
        messages: [
          {
            ...
            parts: [{ type: 'text', text: 'look at src/components/Button.tsx' }],
          },
        ],
      }
    );
    ...
    await chatMessage({ sessionID: 'ses_prose', messageID: 'msg_prose' }, output);
    expect(output.parts.filter((p) => p.synthetic)).toHaveLength(0);
  });
...
Read-content guidance.`
    ...
    await chatMessage({ sessionID: 'ses_read', messageID: 'msg_read_1' }, output);
    const syntheticText = output.parts
      .filter((p) => p.synthetic)
      .map((p) => p.text)
      .join('\n');
...
Use React best practices for components.`
    ...
    await chatMessage({ sessionID: testSessionID, messageID: 'msg_negctx_1' }, output);
...
Special rule content.`
    ...
    await chatMessage({ sessionID: testSessionID, messageID: 'msg_uncond_1' }, output);
...
describe('Session compacting behavior', () => {
  ...
  it('sorts context paths deterministically using lexicographic order', async () => {
    ...
    expect(pathMatches).toEqual(['src/alpha.ts', 'src/Beta.ts', 'src/gamma.ts', 'src/zebra.ts']);
  });
...
describe('Synthetic-part delivery lifecycle', () => {
  ...
  it('full turn: rules persist once, hook fires mid-turn, delivered transiently then durably', async () => {
    ...
    const mockInput = createMockPluginInput({ testDir });
    const hooks = await createHooksWithMatchedRulesStateStore(mockInput, matchedRulesStateStore);
    const chatMessage = hooks['chat.message'] as (
      input: { sessionID: string; messageID?: string },
      output: {
        message: { role: string };
    ...
    await chatMessage({ sessionID: 'ses_life', messageID: 'msg_life_1' }, turn1);
    ...
    expect(turn1Synthetic[0]?.text).toContain('<rule name="lint-hook">\nMind the linter.\n</rule>');
    ...
    await chatMessage({ sessionID: 'ses_life', messageID: 'msg_life_2' }, turn2);
    ...
    const durableHook = turn2.parts.find((p) => p.synthetic && p.id?.startsWith('prt_rules_'));
    expect(durableHook?.text).toContain('<rule name="lint-hook">\nMind the linter.\n</rule>');
  ...
  it('compaction: durable rules are re-appended when compacted history omits them', async () => {
    ...
    await chatMessage({ sessionID: 'ses_comp', messageID: 'msg_comp_1' }, turn1);
    ...
    await chatMessage({ sessionID: 'ses_comp', messageID: 'msg_comp_2' }, turn2);
    ...
    await chatMessage({ sessionID: 'ses_comp', messageID: 'msg_comp_3' }, turn3);
  ...
  it('re-appends durable rules when their owning message is removed', async () => {
    ...
    await chatMessage({ sessionID: 'ses_removed', messageID: 'msg_removed' }, first);
    ...
    await chatMessage({ sessionID: 'ses_removed', messageID: 'msg_replacement' }, replacement);
