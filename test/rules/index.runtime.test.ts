...
import * as ruleDiscoveryModule from '../../src/rules/rule-discovery.js';
import * as ruleMetadataModule from '../../src/rules/rule-metadata.js';
import * as ruleFilterModule from '../../src/rules/rule-filter.js';
import * as messagePathsModule from '../../src/rules/message-paths.js';
import * as utilsModule from '../../src/rules/utils.js';
...
describe('module boundary tests', () => {
  it('should re-export discoverRuleFiles from rule-discovery module', () => {
    ...
    expect(utilsModule.discoverRuleFiles).toBe(ruleDiscoveryModule.discoverRuleFiles);
  });
  ...
  it('should re-export parseRuleMetadata from rule-metadata module', () => {
    ...
    expect(utilsModule.parseRuleMetadata).toBe(ruleMetadataModule.parseRuleMetadata);
  });
  ...
  it('should re-export promptMatchesKeywords and toolsMatchAvailable from rule-filter module', () => {
    ...
    expect(utilsModule.promptMatchesKeywords).toBe(ruleFilterModule.promptMatchesKeywords);
    expect(utilsModule.toolsMatchAvailable).toBe(ruleFilterModule.toolsMatchAvailable);
  });
  ...
  it('should re-export extractFilePathsFromMessages from message-paths module', () => {
    ...
    expect(typeof messagePathsModule.extractFilePathsFromMessages).toBe('function');
  ...
  it('should re-export evaluateHooks and serializeToolArgs from rule-hooks module', () => {
    ...
    expect(utilsModule.serializeToolArgs).toBe(ruleHooksModule.serializeToolArgs);
  });
});
...
describe('OpenCodeRulesPlugin', () => {
  ...
  it('should export a plugin module with id and server', async () => {
    const { default: pluginModule } = await import('../../src/rules/index.ts');
...
describe('SessionState', () => {
  ...
  it('includes glob-conditional rule when the after-hook records a matching file', async () => {
    ...
    expect(output.parts.filter((p) => p.synthetic)[0]?.text).toContain('React best practices');
  });
  ...
  it('supported file tools produce observations; excluded tools do not', async () => {
    ...
    expect(snapshotAfter?.workingContextPaths.has('src/legacy/module.ts')).toBe(true);
    ...
    await chatMessage({ sessionID: 'ses_glob_live', messageID: 'msg_ses_glob_live_1' }, output);

    expect(output.parts.filter((p) => p.synthetic)[0]?.text).toContain('Legacy module guidance');
  });
});
...
describe('history scan and rescan', () => {
  ...
  it('does not let a history rescan overwrite the last complete matched state', async () => {
    const { testDir } = getTestDirs();
    process.env.XDG_CONFIG_HOME = path.join(testDir, '.config');
    await matchedRulesStateStore.write('ses_state_reconcile', ['/rules/current.mdc']);
  ...
  it('recomputes ephemeral rules after compaction without persisting them', async () => {
    ...
    await chatMessage({ sessionID: 'ses_comp_eph', messageID: 'msg_ce_1' }, output);
  ...
  it('seeds Working context from history once and shares the read with delivery', async () => {
    ...
    await chatMessage({ sessionID: 'ses_shared_read', messageID: 'msg_shared_1' }, first);

    // One history read seeded Working context and fed delivery's ledger.
    expect(historyReads).toBe(1);
    expect(first.parts.filter((p) => p.synthetic)[0]?.text).toContain('Seeded rule.');
  });
  ...
  it('message removal does not subtract paths and still re-delivers durable rules', async () => {
    ...
    await chatMessage({ sessionID: 'ses_removal_paths', messageID: 'msg_after_removal' }, output);
    expect(output.parts.filter((p) => p.synthetic)[0]?.text).toContain('Kept-directory guidance.');
  });
  ...
  it('compaction preserves Working context for later matching without re-delivering durable rules', async () => {
    ...
    expect(
      outputMessage.parts.some(
        (part) => part.synthetic && part.text?.includes('Post-compaction guidance.')
      )
    ).toBe(true);
  });
});
...
describe('Matched rules state persistence', () => {
  ...
  it('writes matched rule paths to state file when rules match', async () => {
    ...
    expect(output.parts.filter((p) => p.synthetic)[0]?.text).toContain('Always Apply');
...
describe('matched-rules-state runtime exports', () => {
  it('exports only the store and reader at runtime', () => {
    ...
    expect(exportedKeys).toEqual(['MatchedRulesStateStore', 'readMatchedRulesState']);
  });
});
