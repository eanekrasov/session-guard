...
import { getGitBranch } from '../../src/rules/git-branch.js';

type ExecFileCallback = (error: Error | null, stdout: string, stderr: string) => void;

describe('getGitBranch', () => {
  it('calls execFile with git binary and correct argv', async () => {
    const mockExecFile = vi.fn().mockImplementation((file, args, _opts, callback) => {
  ...
  it('passes cwd, timeout, and killSignal in options', async () => {
    const mockExecFile = vi.fn().mockImplementation((_file, _args, opts, callback) => {
    ...
    await getGitBranch('/my/project/dir', mockExecFile as unknown as typeof execFile);
  });

  it('returns current branch name when git succeeds', async () => {
    const mockExecFile = vi.fn().mockImplementation((_file, _args, _opts, callback) => {
      (callback as ExecFileCallback)(null, 'main\n', '');
      return {} as ReturnType<typeof execFile>;
    });

    const branch = await getGitBranch('/project', mockExecFile as unknown as typeof execFile);
  ...
  it('returns null if not a git repository', async () => {
    ...
    const branch = await getGitBranch('/not-a-repo', mockExecFile as unknown as typeof execFile);
