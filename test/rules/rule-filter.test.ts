...
} from '../../src/rules/rule-filter.js';
import { loadRuleSnapshots } from '../../src/rules/rule-discovery.js';
import type { RuleSnapshot } from '../../src/rules/rule-discovery.js';
import type { RuleMetadata } from '../../src/rules/rule-metadata.js';
import { setupTestDirs, teardownTestDirs } from '../../src/rules/test-fixtures.js';

const snapshot = (metadata: RuleMetadata | null, relativePath = 'test.mdc'): RuleSnapshot => ({
...
describe('rule lifetime classification', () => {
  it.each([
    ...
    ['branch', 'ephemeral'],
    ['tools', 'ephemeral'],
  ] as Array<[RuleConditionKind, 'durable' | 'ephemeral']>)('%s is %s', (kind, expected) => {
    expect(classifyRuleLifetime('any', [{ kind, matched: true, lifetime: expected }])).toBe(
      expected
    );
  });
  ...
  it('classifies a mixed any rule as ephemeral when only the agent matches', () => {
    const [entry] = matchRuleSnapshots([snapshot({ globs: ['src/**/*.ts'], agent: ['plan'] })], {
...
describe('matchRuleSnapshots condition semantics (live delivery)', () => {
  ...
  it('matches path-glob conditions, including matchBase for slash-free patterns', () => {
    const scoped = [snapshot({ globs: ['src/components/**/*.tsx'] }, 'tsx.mdc')];

    const [matched] = matchRuleSnapshots(scoped, {
      fileObservations: [{ path: 'src/components/Button.tsx', tool: 'read', content: '' }],
    });
  ...
  it('matches model, agent, and command conditions', () => {
    ...
    expect(entries.map((entry) => entry.lifetime)).toEqual(['ephemeral', 'ephemeral', 'durable']);
  });

  it('defaults to match: any across mixed durable and ephemeral conditions', () => {
    const entries = matchRuleSnapshots(
      [snapshot({ globs: ['**/*.ts'], model: ['claude-opus'] }, 'mixed-any.mdc')],
      {
        fileObservations: [{ path: 'src/index.ts', tool: 'read', content: '' }],
        modelID: 'gpt-5',
      }
  ...
  it('requires every condition under match: all and stays ephemeral for ephemeral parts', () => {
    const snapshots = [
      snapshot({ keywords: ['refactor'], agent: ['programmer'], match: 'all' }, 'all.mdc'),
    ];
...
describe('file-observation family matching', () => {
  ...
  it('matches fileContains alone against observation content', () => {
    const [entry] = matchRuleSnapshots([snapshot({ fileContains: ['unsafe {'] })], {
      fileObservations: [
        {
          path: 'src/lib.rs',
          tool: 'read',
  ...
  it('matches when one observation satisfies both path and content', () => {
    const [entry] = matchRuleSnapshots(
      [snapshot({ globs: ['**/*.rs'], fileContains: ['unsafe {'] })],
      {
        fileObservations: [{ path: 'src/lib.rs', tool: 'read', content: 'unsafe { }' }],
      }
    );
    expect(entry?.lifetime).toBe('durable');
  });
  ...
  it('treats the family as one check in the match: any algebra', () => {
    ...
    expect(entry?.conditionResults.map((c) => c.kind)).toEqual(['fileContains', 'keywords']);
    expect(entry?.lifetime).toBe('durable');
  });
  ...
  it('matches across multiple literals with OR semantics', () => {
    const [entry] = matchRuleSnapshots([snapshot({ fileContains: ['alpha', 'beta'] })], {
      fileObservations: [{ path: 'a.ts', tool: 'write', content: 'beta here' }],
    });
