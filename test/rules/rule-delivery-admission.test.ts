...
import type { DeliveryPart } from '../../src/rules/rule-delivery-codec.js';
import { ruleKeyFor } from '../../src/rules/rule-delivery-codec.js';
...
function makeHarness(history: RawHistoryResult = { ok: true, messages: [] }): {
  ...
  persist: (impl: (sessionID: string, part: DeliveryPart) => Promise<void>) => void;
} {
  const persisted: DeliveryPart[] = [];
  let impl: (sessionID: string, part: DeliveryPart) => Promise<void> = async () => undefined;
...
describe('durable admission', () => {
  it('persists admitted rules once with admission metadata', async () => {
    ...
    const result = await harness.delivery.admitDurableMatches({
      sessionID: 'ses_admit',
      rules: [ruleA, { relativePath: 'rules/rust-unsafe.md', content: 'Duplicate body.' }],
    });
  ...
  it('retains pending rules on persistence failure and retries succeed', async () => {
    ...
    expect(harness.persisted).toHaveLength(1);
    expect(harness.persisted[0]?.metadata?.ruleKeys).toEqual(ADMISSION_RULE_KEYS);
