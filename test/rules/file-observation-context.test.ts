...
import { createFileObservationContext } from '../../src/rules/file-observation-context.js';
...
describe('FileObservationContext', () => {
  ...
  it('evicts the least-recently-used session at the bound', () => {
    ...
    context.recordObservations('ses_a', [{ path: 'a.ts', tool: 'read', content: 'a' }]);
    context.recordObservations('ses_b', [{ path: 'b.ts', tool: 'read', content: 'b' }]);
    context.getForMatching('ses_a');
    context.recordObservations('ses_c', [{ path: 'c.ts', tool: 'read', content: 'c' }]);
    expect(context.getForMatching('ses_a')).toEqual([{ path: 'a.ts', tool: 'read', content: 'a' }]);
    expect(context.getForMatching('ses_c')).toEqual([{ path: 'c.ts', tool: 'read', content: 'c' }]);
