...
Model-specific guidelines.`
    ...
    const mockClient = { tool: { ids: fn(async () => ({ data: [] })) } };
...
Planning guidelines.`
    ...
    await chatMessage({ sessionID: 'ses_cmd_test', messageID: 'msg_cmd_test_1' }, output);
...
CI-authoritative guidelines.`
    ...
    await chatMessage({ sessionID: 'ses_ci_auth', messageID: 'msg_ci_auth_1' }, message);
...
Node.js project guidelines.`
    ...
    await chatMessage({ sessionID: 'ses_proj_tags', messageID: 'msg_proj_tags_1' }, message);
...
Feature branch guidelines.`
    ...
    const gitBranchModule = await import('../../src/rules/git-branch.ts');
    ...
    try {
      ...
      await chatMessage({ sessionID: 'ses_branch', messageID: 'msg_branch_1' }, message);
  ...
  it('should suppress warnings via console.warn for tool query failures', async () => {
    const { testDir, globalRulesDir } = getTestDirs();
    writeFileSync(path.join(globalRulesDir, 'unconditional.md'), 'Always apply.');
    process.env.XDG_CONFIG_HOME = path.join(testDir, '.config');
  ...
  it('should not throw when project tags detection fails', async () => {
    const { testDir, globalRulesDir } = getTestDirs();
    writeFileSync(path.join(globalRulesDir, 'unconditional.md'), 'Always apply this rule.');
    process.env.XDG_CONFIG_HOME = path.join(testDir, '.config');
    ...
    await chatMessage({ sessionID: 'ses_tags_fail', messageID: 'msg_tags_fail_1' }, message);
  ...
  it('should not throw when git branch detection fails', async () => {
    ...
    await chatMessage({ sessionID: 'ses_branch_fail', messageID: 'msg_branch_fail_1' }, message);
