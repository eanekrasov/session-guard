...
} from '../../src/rules/matched-rules-state.js';
import { buildDurableDeliveryPart } from '../../src/rules/rule-delivery-codec.js';
import { clearRuleCache } from '../../src/rules/utils.js';
...
describe('chat.message rule persistence', () => {
  ...
  afterEach(async () => {
    teardownTestDirs();
    // no resetAllMocks;
    const { __testOnly } = await import('../../src/rules/index.ts');
  ...
  async function getHooks(testDir: string) {
    return createHooksWithStore(createMockPluginInput({ testDir }), matchedRulesStateStore);
  }
  ...
  it('appends all matched rules as one named delivery event', async () => {
    ...
    await chatMessage({ sessionID: 'ses_append', messageID: 'msg_append_1' }, output);
  ...
  it('stamps sessionID and messageID onto appended synthetic parts', async () => {
    ...
    await chatMessage({ sessionID: 'ses_stamp', messageID: 'msg_host_1' }, output);
  ...
  it('deduplicates rules already injected on earlier messages', async () => {
    ...
    } = await import('../../src/rules/index.ts');
    ...
    const hooks = await plugin(mockInput as unknown as Parameters<typeof plugin>[0]);
    const chatMessage = hooks['chat.message'] as HookChatMessage;
    ...
    await chatMessage({ sessionID: 'ses_dedup', messageID: 'msg_dedup_1' }, first);
    ...
    await chatMessage({ sessionID: 'ses_dedup', messageID: 'msg_dedup_2' }, second);
  ...
  it('restores supported tool paths from history without replaying matching', async () => {
    ...
    await chatMessage({ sessionID: 'ses_bash_restart', messageID: 'msg_bash_restart' }, output);
  ...
  it('keeps the original rule content after an in-process file edit', async () => {
    ...
    await chatMessage({ sessionID: 'ses_change', messageID: 'msg_change_1' }, first);
    expect(first.parts.some((part) => part.text?.includes('Version one.'))).toBe(true);
    ...
    await chatMessage({ sessionID: 'ses_change', messageID: 'msg_change_2' }, second);

    expect(second.parts.some((part) => part.text?.includes('Version two.'))).toBe(false);
  ...
  it('keeps agent rules transient while persisting task rules', async () => {
    ...
    await chatMessage({ sessionID: 'ses_route', messageID: 'msg_plan' }, planOutput);

    expect(planOutput.parts.filter((part) => part.synthetic)).toHaveLength(1);
    expect(planOutput.parts.some((part) => part.text?.includes('Plan-only'))).toBe(false);
    expect(planOutput.parts.some((part) => part.text?.includes('Testing guidance'))).toBe(true);
    ...
    await chatMessage({ sessionID: 'ses_route', messageID: 'msg_build' }, buildOutput);
  ...
  it('skips rule matching but still flushes hooks for text-less messages', async () => {
    ...
    await chatMessage({ sessionID: 'ses_textless', messageID: 'msg_textless_1' }, output);

    const syntheticIds = output.parts.filter((p) => p.synthetic).map((p) => p.id ?? '');
    expect(syntheticIds.filter((id) => id.startsWith('prt_rules_'))).toHaveLength(1);
    expect(output.parts.find((part) => part.id?.startsWith('prt_rules_'))?.text).toContain(
      'Hook rule body.'
    );
  });
  ...
  it('still delivers queued transient Hook content when rule evaluation fails', async () => {
    ...
    const part = (messages[1]!.parts as Array<{ id?: string; text?: string }>)[0];
  ...
  it('deduplicates against history fetched from the client on first message', async () => {
    const { testDir, globalRulesDir } = getTestDirs();
    writeFileSync(path.join(globalRulesDir, 'persisted.md'), 'Persisted rule body.');
    process.env.XDG_CONFIG_HOME = path.join(testDir, '.config');
    ...
    await chatMessage({ sessionID: 'ses_restart', messageID: 'msg_restart_1' }, output);
  ...
  it('does not project matched rules when the durable delivery is rejected', async () => {
    ...
    await chatMessage({ sessionID: 'ses_fetchfail', messageID: 'msg_fetchfail_1' }, output);

    expect(output.parts.filter((p) => p.synthetic)).toHaveLength(0);
    expect(await readMatchedRulesState('ses_fetchfail', { stateDir })).toBeNull();
  });
  ...
  it('invokes session.messages with its receiver so sdk methods stay bound', async () => {
    ...
    mockInput.client.session = sessionApi as unknown as typeof mockInput.client.session;
    ...
    await chatMessage({ sessionID: 'ses_receiver', messageID: 'msg_receiver_1' }, output);
  ...
  it('does not persist hook text owned by an ephemeral rule', async () => {
    ...
    await chatMessage({ sessionID: 'ses_hook_eph', messageID: 'msg_hook_next' }, nextUserMessage);
    expect(nextUserMessage.parts.some((part) => part.text === 'Plan hook guidance.')).toBe(false);
  });
  ...
  it('keeps a mixed any hook durable when a durable condition matches', async () => {
    ...
    await chatMessage({ sessionID: 'ses_hook_mixed', messageID: 'msg_hook_mixed_1' }, output);
    expect(
      output.parts.some(
        (part) =>
          part.synthetic &&
  ...
  it('uses the original snapshot body for hook text after an in-process edit', async () => {
    ...
    await chatMessage({ sessionID: 'ses_hook_edit', messageID: 'msg_he_1' }, first);
    expect(first.parts.some((part) => part.text?.includes('Version one.'))).toBe(true);
    ...
    await chatMessage({ sessionID: 'ses_hook_edit', messageID: 'msg_he_2' }, second);
    const hookPart = second.parts.find((p) => p.synthetic && p.text?.includes('Version one.'));
