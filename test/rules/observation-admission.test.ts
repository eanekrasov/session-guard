...
describe('observation admission and noReply persistence', () => {
  ...
  it('ignores its own admission message in chat.message', async () => {
    ...
    await chatMessage({ sessionID: 'ses_own_admit', messageID: 'msg_admission' }, output);
  ...
  it('refreshes matched-rules sidebar state after successful admission', async () => {
    ...
    // bun:test has no vi.waitFor; poll manually
    let snapshot = await readMatchedRulesState('ses_sidebar', {
      stateDir: path.join(testDir, 'matched-state'),
    });
    for (
      let i = 0;
      i < 20 && !snapshot?.matchedRulePaths?.some((p) => p.includes('rust-unsafe.mdc'));
      i++
    ) {
      await new Promise((r) => setTimeout(r, 25));
      snapshot = await readMatchedRulesState('ses_sidebar', {
        stateDir: path.join(testDir, 'matched-state'),
      });
    }
    expect(snapshot?.matchedRulePaths).toContainEqual(expect.stringContaining('rust-unsafe.mdc'));
  });
  ...
  it('merges admission state with previously matched rules', async () => {
    ...
    expect(state?.matchedRulePaths).toEqual(expect.arrayContaining([alwaysRulePath, fileRulePath]));
