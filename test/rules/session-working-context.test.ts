...
import { createSessionWorkingContext } from '../../src/rules/session-working-context.js';
import { SessionStore } from '../../src/rules/session-store.js';
import type { RawHistoryResult } from '../../src/rules/rule-delivery-history.js';
import type { MessageWithInfo } from '../../src/rules/message-context.js';

type Upstream = (sessionID: string) => Promise<RawHistoryResult>;
const storesByContext = new WeakMap<object, SessionStore>();

function setUpstream(result: RawHistoryResult | (() => Promise<RawHistoryResult>)): {
  upstream: Upstream;
  calls: () => number;
} {
  let calls = 0;
...
function historyMessage(sessionID: string, parts: Array<Record<string, unknown>>): MessageWithInfo {
...
function setup(opts: SetupOptions = {}) {
  ...
  const { upstream, calls } = setUpstream(opts.history ?? { ok: true, messages: [] });
...
function supplied(parts: Array<Record<string, unknown>>, sessionID = 'ses_1'): MessageWithInfo[] {
  return [
    {
...
const observedPaths = (
  ...
  Array.from(storesByContext.get(wc)?.snapshot(sessionID)?.workingContextPaths ?? []).sort((a, b) =>
    a.localeCompare(b)
  );
...
describe('supplied-message seeding', () => {
  it('seeds paths from supplied transform messages', async () => {
    const { wc } = setup();
    await wc.seedFromSuppliedMessages('ses_1', supplied([readInvocation('src/a.ts')]));
  ...
  it('reports whether it performed the seeding', async () => {
    const { wc } = setup();
    expect(await wc.seedFromSuppliedMessages('ses_seed', supplied([]))).toBe(true);
    expect(await wc.seedFromSuppliedMessages('ses_seed', supplied([]))).toBe(false);
  });

  it('normalizes and deduplicates paths across seeds', async () => {
    const { wc } = setup();
    await wc.seedFromSuppliedMessages('ses_1', supplied([readInvocation('/proj/src/a.ts')]));
    await wc.seedFromSuppliedMessages('ses_1', supplied([readInvocation('src/a.ts')]));
    expect(observedPaths(wc, 'ses_1')).toEqual(['src/a.ts']);
  });
  ...
  it('exposes detached sorted paths; mutating the view does not alter storage', async () => {
    ...
    expect(observedPaths(wc, 'ses_1')).toEqual(['src/alpha.ts', 'src/zebra.ts']);
  });
});
...
describe('live accumulation', () => {
  it('accumulates tool-call observations monotonically', () => {
    const { wc } = setup();
    wc.recordObservations('ses_1', [{ path: 'src/a.ts', tool: 'read', content: '' }]);
    expect(observedPaths(wc, 'ses_1')).toEqual(['src/a.ts']);
  });

  it('stores only paths from live observations', () => {
    const { wc } = setup();
    wc.recordObservations('ses_1', [{ path: 'src/lib.rs', tool: 'write', content: 'unsafe { }' }]);
  ...
  it('records paths from the current message parts', () => {
    ...
    expect(observedPaths(wc, 'ses_1')).toEqual(['src/attached.yml', 'src/x.ts']);
  });
...
describe('durable-turn preparation', () => {
  it('does not reseed after supplied-message seeding', async () => {
    ...
    await wc.seedFromSuppliedMessages('ses_1', supplied([readInvocation('src/live.ts')]));
    await wc.prepareDurableTurn('ses_1');
    await wc.prepareDurableTurn('ses_1');
    expect(calls()).toBe(0);
  ...
  it('fails open on rejected history and allows a later retry', async () => {
    ...
    wc.recordObservations('ses_1', [{ path: 'src/live.ts', tool: 'read', content: '' }]);
    await wc.prepareDurableTurn('ses_1');
    expect(observedPaths(wc, 'ses_1')).toEqual(['src/live.ts']);
    await wc.prepareDurableTurn('ses_1');
    expect(observedPaths(wc, 'ses_1')).toEqual(['src/live.ts']);
...
describe('history invalidation', () => {
  it('monotonic paths survive invalidation', async () => {
    ...
    expect(observedPaths(wc, 'ses_1')).toEqual(expect.arrayContaining(['src/h.ts', 'src/live.ts']));
    expect(calls()).toBe(1);
  });
...
describe('prefetch retention bounds', () => {
  ...
  it('evicting a prefetch does not alter Working context', async () => {
    ...
    expect(observedPaths(swc.workingContext, 'ses_evicted')).toEqual(remembered);
  });
});
...
describe('concurrent preparation', () => {
  it('shares one in-flight read per session', async () => {
    ...
    await Promise.all([wc.prepareDurableTurn('ses_1'), wc.prepareDurableTurn('ses_1')]);
    expect(calls()).toBe(1);
    expect(observedPaths(wc, 'ses_1')).toEqual(['src/h.ts']);
  });
  ...
  it('keeps different sessions independent', async () => {
    ...
    await Promise.all([wc.prepareDurableTurn('ses_a'), wc.prepareDurableTurn('ses_b')]);
    expect(calls()).toBe(2);
  });
});
