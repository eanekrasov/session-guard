...
import { extractToolCallPaths } from '../../src/rules/message-paths.js';
import {
  setupTestDirs,
  teardownTestDirs,
  getTestDirs,
...
describe('extractFilePathsFromMessages', () => {
  ...
  it('should combine and deduplicate legacy and current tool parts', () => {
    ...
    expect(paths).toEqual(expect.arrayContaining(['src/utils.ts', 'src/new.ts']));
  });
...
describe('extractToolCallPaths', () => {
  it('maps read/edit/write to filePath', () => {
    for (const tool of ['read', 'edit', 'write']) {
      expect(extractToolCallPaths(tool, { filePath: 'src/a.ts' })).toEqual(['src/a.ts']);
    }
  });
  ...
  it('maps glob pattern-only calls to the pattern-derived directory', () => {
    expect(extractToolCallPaths('glob', { pattern: 'src/lib/*' })).toEqual(['src/lib']);
  });

  it('maps bash to workdir', () => {
    expect(extractToolCallPaths('bash', { command: 'ls', workdir: 'src/tools' })).toEqual([
      'src/tools',
    ]);
  });
  ...
  it('returns nothing for Object.prototype property names', () => {
    expect(extractToolCallPaths('constructor', { filePath: 'src/a.ts' })).toEqual([]);
  ...
  it('extracts bash workdirs from history parts', () => {
    ...
    expect(extractFilePathsFromMessages(currentMessages)).toContain('src/tools');
  });
...
describe('promptMatchesKeywords', () => {
  it('should return true when keyword matches prompt', () => {
    expect(promptMatchesKeywords('I need help testing this function', ['testing'])).toBe(true);
  });

  it('should return false when keyword does not match prompt', () => {
    expect(promptMatchesKeywords('help me with the database', ['testing', 'jest'])).toBe(false);
  });
  ...
  it('should handle multi-word keywords', () => {
    expect(promptMatchesKeywords('I need help with unit test coverage', ['unit test'])).toBe(true);
  });

  it('should return true if any keyword matches (OR logic)', () => {
    expect(promptMatchesKeywords('please help with jest', ['testing', 'jest', 'vitest'])).toBe(
      true
    );
  });
...
describe('toolsMatchAvailable', () => {
  ...
  it('should return true if any required tool is available (OR logic)', () => {
    const available = ['mcp_bash', 'mcp_read'];
    expect(toolsMatchAvailable(available, ['mcp_websearch', 'mcp_bash'])).toBe(true);
  });
...
describe('discoverRuleFiles', () => {
  ...
  describe('global rules discovery', () => {
    it('should discover markdown files from XDG_CONFIG_HOME/opencode/rules', async () => {
      ...
      expect(files.some((f) => f.filePath === path.join(globalRulesDir, 'rule1.md'))).toBe(true);
      expect(files.some((f) => f.filePath === path.join(globalRulesDir, 'rule2.md'))).toBe(true);
    });
    ...
    it('should use ~/.config/opencode/rules as fallback when XDG_CONFIG_HOME not set', async () => {
      ...
      const files = await discoverRuleFiles();
      expect(files.some((f) => f.filePath === path.join(fallbackDir, 'rule.md'))).toBe(true);
    });
    ...
    it('should include both .md and .mdc files', async () => {
      ...
      writeFileSync(path.join(globalRulesDir, 'rule.mdc'), '# Rule with metadata');
    ...
    it('should use OPENCODE_CONFIG_DIR/rules as global dir when set', async () => {
      ...
      const files = await discoverRuleFiles();
      expect(files.some((f) => f.filePath === path.join(customRulesDir, 'custom.md'))).toBe(true);
    });
  ...
  describe('project rules discovery', () => {
    it('should discover markdown files from .opencode/rules directory', async () => {
      ...
      expect(files.some((f) => f.filePath === path.join(projRulesDir, 'local-rule.md'))).toBe(true);
    });
  ...
  describe('subdirectory scanning', () => {
    it('should discover rules in nested subdirectories', async () => {
      ...
      const files = await discoverRuleFiles();
      expect(files.some((f) => f.filePath === path.join(nestedDir, 'react.md'))).toBe(true);
    });

    it('should discover rules in deeply nested subdirectories (multiple levels)', async () => {
      const { testDir, globalRulesDir } = getTestDirs();
      const deepDir = path.join(globalRulesDir, 'lang', 'typescript', 'framework');
      ...
      const files = await discoverRuleFiles();
      expect(files.some((f) => f.filePath === path.join(deepDir, 'nextjs.md'))).toBe(true);
    });
    ...
    it('should exclude hidden subdirectories', async () => {
      ...
      expect(files.some((f) => f.filePath === path.join(globalRulesDir, 'visible.md'))).toBe(true);
    });
    ...
    it('should discover rules from mixed flat and nested structures', async () => {
      ...
      const files = await discoverRuleFiles();
      expect(files).toHaveLength(2);
      expect(files.some((f) => f.filePath === path.join(globalRulesDir, 'root.md'))).toBe(true);
      expect(files.some((f) => f.filePath === path.join(nestedDir, 'child.md'))).toBe(true);
    });
  ...
  describe('ENOENT handling', () => {
    it('should handle missing directories gracefully without warnings', async () => {
      ...
      try {
        const files = await discoverRuleFiles();
        expect(files).toEqual([]);
        expect(warnings.filter((w) => w.includes('opencode-rules'))).toHaveLength(0);
      } finally {
    ...
    it('should handle race condition where directory is deleted mid-scan', async () => {
      ...
      try {
        const files = await discoverRuleFiles();
        expect(warnings.filter((w) => w.includes('ENOENT') || w.includes('no such'))).toHaveLength(
          0
        );
        expect(files).toEqual([]);
      } finally {
...
Rule content`;
    ...
    expect(metadata?.keywords).toEqual(['refactoring', 'cleanup', 'code review']);
  });
