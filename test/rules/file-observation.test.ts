...
import { normalizeObservations, type RawToolEvent } from '../../src/rules/file-observation.js';
...
describe('normalizeToolObservation: unsupported tools', () => {
  it('yields no observation for grep, glob, bash, and unknown tools', () => {
    for (const tool of ['grep', 'glob', 'bash', 'webfetch', 'custom_tool']) {
      expect(
        normalizeObservations(readEvent({ tool, args: { filePath: 'src/a.ts' }, output: 'text' }))
      ).toEqual([]);
    }
  });

  it('yields no observation for failed calls or empty paths', () => {
    expect(normalizeObservations(readEvent({ args: { filePath: '' } }))).toEqual([]);
    expect(normalizeObservations(readEvent({ args: { filePath: 42 } }))).toEqual([]);
...
describe('normalizeToolObservation: write', () => {
  ...
  it('yields a path-only observation when content is not a string', () => {
    expect(
      normalizeObservations(readEvent({ tool: 'write', args: { filePath: 'src/a.ts' } }))
...
describe('normalizeToolObservation: edit', () => {
  ...
  it('degrades malformed old/new arguments to a path-only observation', () => {
    expect(
      normalizeObservations(readEvent({ tool: 'edit', args: { filePath: 'src/a.ts' } }))
