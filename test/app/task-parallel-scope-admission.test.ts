...
function setParallelProfilesDir(): void {
  const profilesDir = mkdtempSync(join(tmpdir(), 'parallel-scope-profiles-'));
  cleanupDirs.push(profilesDir);
  const profileDir = join(profilesDir, 'parallel-scope');
  mkdirSync(profileDir, { recursive: true });
  writeFileSync(
    join(profileDir, 'profile.json'),
    JSON.stringify({ id: 'parallel-scope', name: 'parallel-scope', schemas: ['cycle.yaml'] }),
    'utf-8'
  );
  writeFileSync(
