...
} from '../../../src/tui/data/rules-load-coordinator.ts';
import type { LoadSidebarRulesResult } from '../../../src/tui/data/rules.ts';
...
describe('createRulesLoadCoordinator', () => {
  ...
  it('retries fallback state until evaluated state is available', async () => {
    ...
    await flushPromises();
    expect(published.map((value) => value.hasEvaluationState)).toEqual([false, true]);
  ...
  it('settles rejected loads so a later refresh can run', async () => {
    const load = vi.fn((_target: RulesLoadTarget): Promise<LoadSidebarRulesResult> =>
