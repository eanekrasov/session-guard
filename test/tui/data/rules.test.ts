...
import { clearRuleCache } from '../../../src/rules/rule-discovery.ts';
import {
  classifyRuleScope,
  hasConditions,
...
} from '../../../src/tui/data/rules.ts';

const originalDebugEnv = (() => {
...
describe('classifyRuleScope', () => {
  it('returns "global" when projectDir is null', () => {
    expect(classifyRuleScope('/home/user/.config/opencode/rules/foo.md', null)).toBe('global');
  });

  it('returns "project" for files under projectDir/.opencode/rules/', () => {
    expect(classifyRuleScope('/project/.opencode/rules/foo.md', '/project')).toBe('project');
  });

  it('returns "project" for files in subdirectories under project rules', () => {
    expect(classifyRuleScope('/project/.opencode/rules/sub/deep.md', '/project')).toBe('project');
  });

  it('returns "global" for files not under projectDir/.opencode/rules/', () => {
    expect(classifyRuleScope('/home/user/.config/opencode/rules/foo.md', '/project')).toBe(
      'global'
    );
  });
  ...
  it('does not match partial path prefixes', () => {
    ...
    expect(classifyRuleScope('/project/.opencode/rules-extra/foo.md', '/project')).toBe('global');
  });
});
...
describe('formatConditionSummary', () => {
  it('formats single array field', () => {
    expect(formatConditionSummary({ globs: ['**/*.ts'] })).toBe('globs: **/*.ts');
  });
  ...
  it('formats fileContains literals', () => {
    expect(formatConditionSummary({ fileContains: ['unsafe {'] })).toBe('fileContains: unsafe {');
  });
  ...
  it('formats all fields in canonical order', () => {
    ...
    expect(result).toBe('globs: *.md, fileContains: TODO, os: linux, ci: false, match: all');
  });
});
...
function makeEntry(overrides: Partial<SidebarRuleEntry> & { path: string }): SidebarRuleEntry {
  return {
    name: '',
    path: overrides.path,
