...
| 9 | A commit that sweeps in an unrelated file is not receipted | **PASS** | 6 | HEAD moved but the commit carried unrelated.txt: no receipt, permit dropped, refusal surfaced |
| 10 | A live subagent closes a gate with its own workflow-result | **PASS** | 6 | two live subagents each closed their own gate with a workflow-result; the stage held for the first and completed the task on the second |
