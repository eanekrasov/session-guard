...
### 8. Git checkout / reset / restore — ТОЛЬКО после проверки состояния

**ЗАПРЕЩЕНО** выполнять `git checkout`, `git restore`, `git reset --hard` на файле без предварительной проверки:

```bash
# ОБЯЗАТЕЛЬНЫЙ ПРОТОКОЛ перед любой деструктивной git-операцией:
git status -- <file>      # 1. Есть ли локальные изменения?
git diff HEAD -- <file>   # 2. Что именно теряем?
```

**Правила решения:**

- Если файл `modified` (есть незакоммиченные правки) — **НЕЛЬЗЯ** делать checkout/reset/restore.
  - Вариант А: `git add -A && git commit -m "wip: <phase>"` — закоммитить, потом checkout безопасен
  - Вариант Б: `git stash` — спрятать, сделать операцию, `git stash pop`
  - Вариант В: вручную перенести нужные изменения, потом checkout

- Если файл `untracked` — `git checkout` его не трогает (безопасно), НО `git clean -f` УДАЛИТ.
  - Проверить: `git status -- <file>` покажет `??` — значит checkout безопасен, clean опасен.

- Если файл в индексе (`staged`) — `git reset HEAD <file>` сначала, потом по правилам выше.

**Никаких исключений.** Даже если "на 99% уверен, что там ничего важного". Проверка занимает 2 секунды. Потеря работы — часы.

### 9. Многофазная рефакторинг-работа — коммит на границе каждой фазы

При выполнении работы, состоящей из последовательных фаз (Phase 1 → Phase 2 → Phase 3...), где каждая фаза меняет одни и те же файлы:

1. **После завершения каждой фазы** — обязательный коммит:

   ```bash
   git add -A && git commit -m "phase N: <what changed>"
   ```

   Это создаёт точку восстановления. Если следующая фаза сломается — `git reset --hard HEAD` безопасен.

2. **Никогда не накапливать незакоммиченные изменения** через границу фаз. Каждая фаза = атомарный коммит.

3. **Если фаза не может быть закончена** — закоммитить как `wip: phase N` с пометкой, что не готово. Не оставлять незакоммиченным.

4. **Перед началом следующей фазы** — `git diff HEAD~1..HEAD` для проверки, что предыдущая фаза зафиксировалась корректно.

Это правило перекрывает пункт 8: коммит делает checkout/reset безопасными, потому что история сохранена.
