...
import { type RawHistoryAdapter, type RawHistoryResult } from './rule-delivery-history.js';
...
export interface RuleDelivery {
  ...
  admitDurableMatches(input: DurableAdmissionInput): Promise<DurableAdmissionResult>;
...
function hasDeliveryKey(keys: ReadonlySet<string>, rule: MatchedRuleContent): boolean {
  return (
...
class DefaultRuleDelivery implements RuleDelivery {
  ...
  async admitDurableMatches(input: DurableAdmissionInput): Promise<DurableAdmissionResult> {
    return this.serialize(input.sessionID, async () => {
      ...
      return (await this.persistPendingRules(input.sessionID, state)) ? 'accepted' : 'pending';
    });
  }

  private async decodeHistory(sessionID: string): Promise<DeliveryLedgerFacts | undefined> {
    let result: RawHistoryResult;
    try {
      result = await this.rawHistory.readHistory(sessionID);
    } catch (error) {
      this.debugLog(`History read failed for ${sessionID}: ${formatError(error)}`);
      return undefined;
    }
  ...
  async deliverDurableTurn(input: DurableTurnInput): Promise<DurableTurnResult> {
    return this.serialize(input.sessionID, async () => {
      try {
        return await this.deliverDurable(input);
      } catch (error) {
        this.debugLog(`Durable delivery failed for ${input.sessionID}: ${formatError(error)}`);
        return 'deferred';
      }
    });
  }

  private async deliverDurable(input: DurableTurnInput): Promise<DurableTurnResult> {
    const state = this.getState(input.sessionID);
    ...
    for (const rule of input.matchedRules) {
      ...
      if (hasDeliveryKey(state.ruleKeys, rule) || newRuleKeys.has(key)) continue;
      newRuleKeys.add(key);
      newRules.push(rule);
    }
    ...
    for (const hook of state.durableHookQueue) {
      ...
      if (hasDeliveryKey(state.hookKeys, hook) || newHookKeys.has(key)) continue;
      newHookKeys.add(key);
      newHooks.push(hook);
    }
    ...
    for (const hook of input.hooks) {
      if (
        state.pendingHookQueue.some(
          (pending) =>
            (pending.identity ?? pending.relativePath) === (hook.identity ?? hook.relativePath)
        ) ||
  ...
  deliverTransientDispatch(input: TransientDispatchInput): void {
    try {
      ...
      const turnID = typeof realUserInfo?.id === 'string' ? realUserInfo.id : undefined;
      ...
      const baseInfo = realUserInfo ?? input.messages[input.messages.length - 1]?.info ?? {};
      ...
      for (const rule of [...input.matchedRules, ...state.pendingRuleQueue]) {
        ...
        if (
          ...
          (transientTurn !== undefined && hasDeliveryKey(transientTurn.ruleKeys, rule))
        ) {
          continue;
        }
      ...
      for (const hook of [...state.durableHookQueue, ...state.transientHookQueue]) {
        const key = deliveryKey(hook);
        if (
          hasDeliveryKey(presentHookKeys, hook) ||
          (transientTurn !== undefined && hasDeliveryKey(transientTurn.hookKeys, hook))
        ) {
          continue;
        }
      ...
      if (transientRules.length > 0 || transientHooks.length > 0) {
        ...
        if (part && !presentIDs.has(transientMessage.info.id) && !presentIDs.has(part.id)) {
          input.messages.push({
      ...
      this.debugLog(`Transient delivery failed for ${input.sessionID}: ${formatError(error)}`);
    }
  }
  ...
  private queuePendingRules(state: DeliveryState, rules: readonly MatchedRuleContent[]): boolean {
    let added = false;
    for (const rule of rules) {
      if (
        hasDeliveryKey(state.ruleKeys, rule) ||
        state.pendingRuleQueue.some((pending) => deliveryKey(pending) === deliveryKey(rule))
      ) {
        continue;
      }
  ...
  private async persistPendingRules(sessionID: string, state: DeliveryState): Promise<boolean> {
    if (!this.persistAdmission || state.pendingRuleQueue.length === 0) {
      return state.pendingRuleQueue.length === 0;
    }
    const pending = state.pendingRuleQueue.filter((rule) => !hasDeliveryKey(state.ruleKeys, rule));
    if (pending.length === 0) {
      state.pendingRuleQueue = [];
      return true;
    }
    try {
      await this.persistAdmission(sessionID, buildRuleAdmissionPart(pending, sessionID));
    } catch (error) {
      this.debugLog(`Rule admission persistence failed for ${sessionID}: ${formatError(error)}`);
      return false;
    }
  ...
  private replaceLedger(state: DeliveryState, facts: DeliveryLedgerFacts): void {
