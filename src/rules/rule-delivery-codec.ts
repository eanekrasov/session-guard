...
function renderSystemMessage(rules: readonly DeliveryRule[]): string {
  const renderedRules = rules
    .map((rule) => `<rule name="${escapeAttribute(shortName(rule))}">\n${rule.content}\n</rule>`)
    .join('\n\n');
  return `<system-message>\n${DELIVERY_PREAMBLE}\n\n${renderedRules}\n</system-message>`;
}

function deliveryIdentity(ruleKeys: readonly string[], hookKeys: readonly string[]): string {
  return shortHash(
    [...ruleKeys.map((key) => `rule:${key}`), ...hookKeys.map((key) => `hook:${key}`)].join('|')
  );
}
...
export function isTransientMessageId(id: unknown): boolean {
  return (
    typeof id === 'string' &&
    (id.startsWith(TRANSIENT_RULE_MESSAGE_PREFIX) || id.startsWith(TRANSIENT_HOOK_MESSAGE_PREFIX))
  );
}

export function decodeRawHistory(messages: readonly unknown[]): DeliveryLedgerFacts {
