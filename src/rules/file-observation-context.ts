import { normalizeObservations, type FileObservation } from './file-observation.js';
...
export function createFileObservationContext(
  ...
  const normalizeForContext = (observation: FileObservation): FileObservation => ({
    ...observation,
    path: normalizeContextPath(observation.path, options.projectDirectory),
  });

  const recordNormalized = (sessionID: string, observations: readonly FileObservation[]): void => {
  ...
  return {
    recordToolEvent: (sessionID, event) => {
      const observations = normalizeObservations(event).map(normalizeForContext);
    ...
    getForMatching: (sessionID) => {
      ...
      if (!session) return [];
      session.lastUpdated = ++tick;
      return session.observations.map((observation) => ({ ...observation })).sort(pathComparator);
    },
  };
}
