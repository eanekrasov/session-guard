import { extractObservationsFromMessageParts, type FileObservation } from './file-observation.js';
import {
...
export interface WorkingContext {
  ...
  seedFromSuppliedMessages(sessionID: string, messages: readonly MessageWithInfo[]): boolean;
  ...
  recordObservations(sessionID: string, observations: readonly FileObservation[]): void;
...
export function createSessionWorkingContext(
  ...
  const addObservations = (
    ...
    sessionStore.upsert(sessionID, (state) => {
      for (const observation of observations) {
        state.workingContextPaths.add(normalizeContextPath(observation.path, projectDirectory));
      }
  ...
  const recordMessageParts = (sessionID: string, parts: readonly unknown[]): void => {
  ...
  const compactionProjection = (sessionID: string): string | undefined => {
    const paths = Array.from(sessionStore.get(sessionID)?.workingContextPaths ?? []);
  ...
  async function prepareDurableTurn(sessionID: string): Promise<void> {
    ...
    const settled = existing ? await existing : await readClientHistory(sessionID);
    ...
    if (settled.result.ok) {
      addObservations(
        sessionID,
        extractObservationsFromSuppliedMessages(settled.result.messages as MessageWithInfo[]),
        true
      );
    }
  }
  ...
  return {
    workingContext: {
      ...
      prepareDurableTurn,
      recordMessageParts,
      recordObservations: (sessionID, observations) => addObservations(sessionID, observations),
...
function extractObservationsFromSuppliedMessages(
  ...
  for (const message of filterValidMessages(
    messages.filter(
      (message): message is MessageWithInfo => typeof message === 'object' && message !== null
    )
  )) {
