...
import type { RuleMetadata } from '../../rules/utils.js';
import { logError } from '../../rules/debug.js';
import {
  TuiPanel,
  TuiSection,
  buildPanelPalette,
...
function RuleSection(props: RuleSectionProps): JSX.Element {
  ...
  const bulletColor = (rule: SidebarRuleEntry): string => {
    return rule.isActive === true ? props.palette.success : props.palette.muted;
  };
  ...
  return props.rules.length > 0 ? (
    <TuiSection
      ...
      open={() => props.open}
      title={props.title}
      suffix={props.open ? undefined : ` (${headerCount()})`}
      ...
      {props.open && (
        <For each={props.rules}>
          {(rule, localIndex) => {
            ...
            return (
              <box flexDirection="column" onMouseDown={() => props.onExpandToggle(globalIndex())}>
                <box flexDirection="row" gap={1}>
                  <text fg={bulletColor(rule)}>•</text>
                  <text fg={props.palette.text}>{rule.name}</text>
                </box>
                {props.expandedIndex === globalIndex() && (
                  <box flexDirection="column" paddingLeft={4}>
                    <text fg={props.palette.muted}>{rule.path}</text>
                    ...
                    {rule.metadata.ci !== undefined && (
                      <text fg={props.palette.muted}>CI: {String(rule.metadata.ci)}</text>
                    )}
                    {rule.metadata.match && (
                      <text fg={props.palette.muted}>Match: {rule.metadata.match}</text>
                    )}
                  </box>
                )}
              </box>
...
export function SidebarContent(props: SidebarContentProps): JSX.Element {
  ...
  const [lastDir, setLastDir] = createSignal<string | null | undefined>(undefined);
  ...
  const triggerRefresh = (event: { type: string; properties: Record<string, unknown> }): void => {
  ...
  const panelLayout = createPanelLayout({ border: createMemo(() => false) });

  return (
    <TuiPanel pal={panelPalette()} border={false} layout={panelLayout}>
      <text fg={panelPalette().primary}>
        <b>Rules</b>
      </text>

      {status() === 'loading' && <text fg={panelPalette().muted}>Loading...</text>}
      {status() === 'error' && <text fg={panelPalette().muted}>Failed to load rules</text>}
      ...
      {status() === 'loaded' && (
        <>
          {rules().length > 0 ? (
            ...
            <text fg={panelPalette().muted}>No rules found</text>
          )}
          {skippedCount() > 0 && (
            <text fg={panelPalette().muted}>{skippedCount()} rules skipped (unreadable)</text>
          )}
        </>
      )}
    </TuiPanel>
