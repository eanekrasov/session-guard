...
export async function loadSidebarRules(
  ...
  const matchedState = sessionId ? await readMatchedRulesState(sessionId, options) : null;
  const hasEvaluationState = matchedState !== null;
  const matchedPathsSet = hasEvaluationState ? new Set(matchedState.matchedRulePaths) : null;
  ...
  const sortPriority = (v: boolean | null): number => (v === true ? 0 : v === null ? 1 : 2);
  entries.sort((a, b) => {
...
export function classifyRuleScope(
  ...
  const projectRulesPrefix = path.join(projectDir, '.opencode', 'rules') + path.sep;
