...
export type RulesRetryScheduler = (callback: () => void, delayMs: number) => () => void;
...
export function createRulesLoadCoordinator(
  ...
  let nextRequestId = 0;
  let activeRequest: { id: number; generation: number; reset: boolean } | undefined;
  ...
  function isCurrent(request: NonNullable<typeof activeRequest>, target: RulesLoadTarget): boolean {
    return (
      !disposed &&
  ...
  function settlePendingRefresh(target: RulesLoadTarget, generation: number): boolean {
  ...
  function startLoad(target: RulesLoadTarget, generation: number, reset: boolean): void {
    if (disposed) return;
    const request = {
      id: ++nextRequestId,
      generation,
